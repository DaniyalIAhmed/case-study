import sequelize, { readDb, redisClient, Op } from "./db.js";
...
const updateProductStock = TryCatch(async (req, res) => {
  ...
  const store = await Store.findOne({
    where: { id: product.storeId, userId: user.id },
  });
  if (!store)
    return res
      .status(403)
      .json({ error: "Unauthorized to update product in this store" });
  ...
  res
    .status(200)
    .json({
      message: "Product quantity updated & stock movement logged",
      product,
    });
});
const getStockReport = TryCatch(async (req, res) => {
  ...
  if (!store)
    return res
      .status(403)
      .json({ error: "Unauthorized to access report for this store" });
...
const getStockReportCached = TryCatch(async (req, res) => {
  const { storeId, startDate, endDate } = req.query;
  const cacheKey = `report:${storeId}:${startDate}:${endDate}`;
